...
SELECT MAX(COST) AS Procedure_Cost
from PROCEDURES
...
from physician P inner join Department D
...
select concat(p.name,'',p.surname) AS PATIENT_NAME, ph.Name as Phy_who_did_Primary_Check
...
From Physician p inner join Department d
on p.employeeid = d.Department_Id
